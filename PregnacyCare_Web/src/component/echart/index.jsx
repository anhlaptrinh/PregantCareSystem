...
const ChartContent = () => {
  ...
  const { data: fetusData = { fetusWeek: [], weight: [], height: [] } } =
    useFetchFetusRecordStatistic(selectedFetus?.id);
  const { data: whoData = { fetusWeek: [], weight: [], height: [] } } =
    useFetchWhoStatistic();
  // const whoData = {
  ...
  const convertedFetusWeight = fetusData.weight;
  const convertedFetusHeight = fetusData.height;
  ...
  const interpolatedFetusWeight = completeWeeks.map((week) => {
    ...
    return interpolate(
      lowerWeek,
      convertedFetusWeight[0],
      upperWeek,
      convertedFetusWeight[1],
      week
    );
  });
  ...
  const interpolatedFetusHeight = completeWeeks.map((week) => {
    ...
    return interpolate(
      lowerWeek,
      convertedFetusHeight[0],
      upperWeek,
      convertedFetusHeight[1],
      week
    );
  });

  // Get WHO data for these weeks
  const matchedWHOWeights = completeWeeks.map(
    (week) => whoData.weight[whoData.fetusWeek.indexOf(week)]
  );
  const matchedWHOHeights = completeWeeks.map(
    (week) => whoData.height[whoData.fetusWeek.indexOf(week)]
  );
  const option = {
    tooltip: {
      trigger: "axis",
