...
const CustomEvent = ({ event }) => {
  ...
  const handleSubmit = async (values, mode) => {
    ...
    localDate.setMinutes(localDate.getMinutes() - localDate.getTimezoneOffset()); // Đảm bảo múi giờ chính xác
  ...
  const handleOpenModal = (event,mode) => {
    ...
    setModalTitle(mode === "Appointment" ? "Update Appointment" : "Create Reminder");
    setModalFields(
      mode === "Appointment"
        ? [
  ...
  <FlexModal
        ...
        onSubmit={(values) => handleSubmit(values, modalTitle.includes("Appointment") ? "Appointment" : "Reminder")}
        fields={modalFields}
        title={modalTitle}
...
const AppointmentCalendar = () => {
  ...
  if (!appointments || !Array.isArray(appointments) || isLoading) return <Spin size="large" />;
